class FilesystemDisplay {
    constructor(opts) {
        ...
        const settingsDir = require("@electron/remote").app.getPath("userData");
        ...
        this.watchFS = (dir) => {
            ...
            this._fsWatcher = fs.watch(dir, (eventType, _filename) => {
                if (eventType != "change") {
        ...
        this.readFS = async (dir) => {
            ...
            if (process.platform === "win32" && dir.endsWith(":"))
                dir = dir + "\\";
            let tcwd = dir;
            let content = await this._asyncFSwrapper
                .readdir(tcwd)
                .catch((_err) => {
        ...
        this.render = async (originBlockList, isDiskView) => {
            ...
            while (this.filesContainer.childNodes[id]) {
                ...
                e.setAttribute(
                    "class",
                    e.className.replace(" animationWait", "")
                );
        ...
        this.renderDiskUsage = async (fsBlock) => {
            ...
            if (!isNaN(fsBlock.use)) {
                this.space_bar.text.innerHTML = `Mount <strong>${window._escapeHtml(displayMount)}</strong> used <strong>${Math.round(fsBlock.use)}%</strong>`;
                ...
                this.space_bar.text.innerHTML = `Mount <strong>${window._escapeHtml(displayMount)}</strong> used <strong>${usage}%</strong>`;
        ...
        if (window.performance.navigation.type === 0) {
            this.readFS(
                window.term[window.currentTerm].cwd || window.settings.cwd
            );
        }

        this.openFile = (name, _path, _type) => {
            ...
            default: {
                if (mime.charset(filetype) === "UTF-8") {
                    fs.readFile(block.path, "utf-8", (_err, _data) => {
                        if (_err) {
                            new Modal({
                                type: "info",
                        ...
                        new Modal(
                            {
                                type: "custom",
                                title: _escapeHtml(name),
                                html: `<textarea id="fileEdit" rows="40" cols="150" spellcheck="false">${_data}</textarea><p id="fedit-status"></p>`,
                                buttons: [
                                    {
                                        label: "Save to Disk",
                                        action: `window.writeFile('${block.path}')`
                                    }
                                ]
                            },
                            () => {
        ...
        this.openMedia = (name, _path, type) => {
            let block, html;
            ...
            case "image": {
                html = `<img class="fsDisp_mediaDisp" src="${window._encodePathURI(_path || block.path)}" ondragstart="return false;">`;
                break;
            }
            case "audio": {
                html = `<div>
                                <div class="media_container" data-fullscreen="false">
                                    <audio class="media fsDisp_mediaDisp" preload="auto">
                                        <source src="${window._encodePathURI(_path || block.path)}">
            ...
            default:
                throw new Error(
                    "fsDisp media displayer: unknown type " +
                            (type || block.type)
                );
            }
