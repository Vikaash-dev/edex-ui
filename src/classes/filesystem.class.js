                ...
                if (!this._noTracking) {
                    if (e.type === "dir" || e.type.endsWith("Dir")) {
                        ...
                        if (process.platform === "win32") {
                            cmd = `window.term[window.currentTerm].writelr("${window._escapeStringForJS(e.path)}")`;
                        } else {
                            cmd = `window.term[window.currentTerm].writelr("cd \\"${window._escapeStringForJS(e.path)}\\"")`;
                        }
                    } else {
                    ...
                    if (e.type === "dir" || e.type.endsWith("Dir")) {
                        ...
                        cmd = `window.fsDisp.readFS("${window._escapeStringForJS(e.path)}")`;
                    } else {
                        cmd = `window.term[window.currentTerm].write("\\""+fsDisp.cwd[${blockIndex}].path+"\\"")`;
                    }
                }
