class Toplist {
    ...
    updateList() {
        ...
        window.si.processes().then((data) => {
            ...
            document
                .querySelectorAll("#mod_toplist_table > tr")
                .forEach((el) => {
                    el.remove();
                });
            list.forEach((proc) => {
        ...
        function updateProcessList() {
            ...
            window.si.processes().then((data) => {
                ...
                data.list.forEach((proc) => {
                    proc.runtime = new Date(
                        Date.now() - Date.parse(proc.started)
                    );
                });
                ...
                let list = data.list.sort((a, b) => {
                    ...
                    case "Started":
                        if (ascending)
                            return (
                                Date.parse(a.started) -
                                    Date.parse(b.started)
                            );
                        else
                            return (
                                Date.parse(b.started) -
                ...
                if (removed) clearInterval(updateInterval);
                    ...
                    document
                        .querySelectorAll("#processList > tr")
                        .forEach((el) => {
                            el.remove();
                        });
